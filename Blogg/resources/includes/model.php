...
if($pdo) {
  ...
  if (isset($_POST['search'])) {
    $order = $_POST['order'];
    if (!empty($_POST['text'])) {
      $where = "WHERE Text LIKE '%{$_POST['text']}%'";
    }
  }

  $sql = "SELECT p.slug, p.Headline, u.Username, p.Creation_Time, p.Text FROM posts AS p JOIN users AS u ON p.User_ID = u.ID {$where} ORDER BY Creation_Time {$order}";
